...
export const db = {
  ...
  getNFTTiers() {
    ...
    return database.prepare('SELECT * FROM nft_tiers ORDER BY id').all();
  },
  ...
  getUserInfo(walletAddress) {
    try {
      ...
      // 🔥 为每个直推成员从 user_nfts 表实时统计 NFT 数据
      const enrichedTeamMembers = teamMembers.map(member => {
        const nftStats = database.prepare(`
          SELECT 
            COUNT(*) as nft_count,
            SUM(COALESCE(un.mint_price, nt.price)) as nft_mint_amount
          FROM user_nfts un
          LEFT JOIN nft_tiers nt ON un.tier_id = nt.id
          WHERE un.user_address = ?
        `).get(member.wallet_address.toLowerCase());
        
        return {
          ...member,
          nft_count: nftStats.nft_count || 0,
          nft_mint_amount: nftStats.nft_mint_amount || 0
        };
      });
      ...
      return {
        exists: true,
        user,
        teamMembers: enrichedTeamMembers, // 直推下级（带实时 NFT 统计）
        teammates    // 战队队友
      };
    } catch (error) {
